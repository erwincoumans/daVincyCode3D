...
void BasicDemo::displayText()
{
	...
	if((getDebugMode() & btIDebugDraw::DBG_DrawText)!=0)
	{
		...
		sprintf(buf,"daVincyCode3D btST2World with PATH in Bullet");
		GLDebugDrawString(xStart,20,buf);
		glRasterPos3f(xStart, yStart, 0);
		sprintf(buf,"Drag objects using left mouse select");
		yStart+=20;
...
void	BasicDemo::initPhysics()
{
	...
	m_dynamicsWorld = new btST2World(m_dispatcher,m_broadphase,m_solver,m_collisionConfiguration);
		...
		for (int k=0;k<ARRAY_SIZE_Y;k++)
		{
			for (int i=0;i<ARRAY_SIZE_X;i++)
			{
				for(int j = 0;j<ARRAY_SIZE_Z;j++)
				{
					...
					{
						btVector3 inertia;
						body->getCollisionShape()->calculateLocalInertia(10000,inertia);
						//body->setMassProps(10000,inertia);
					}
					m_dynamicsWorld->addRigidBody(body);
				}
			}
