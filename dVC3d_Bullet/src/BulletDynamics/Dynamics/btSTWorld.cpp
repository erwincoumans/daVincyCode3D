...
btSTWorld::btSTWorld( btDispatcher* dispatcher,btBroadphaseInterface* pairCache,btConstraintSolver* constraintSolver, btCollisionConfiguration* collisionConfiguration )
...
void btSTWorld::saveKinematicState( btScalar timeStep )
{
...
void btSTWorld::synchronizeSingleMotionState( btRigidBody* body )
{
	btAssert( body );
	...
	if ( body->getMotionState() && !body->isStaticObject() )
	{
		...
		// if (body->getActivationState() != ISLAND_SLEEPING)
		{
			...
			btTransformUtil::integrateTransform( body->getInterpolationWorldTransform(),
												 ...
												 getDispatchInfo().m_timeStep * body->getHitFraction(),
...
void btSTWorld::synchronizeMotionStates()
{
...
int btSTWorld::stepSimulation( btScalar timeStep,int maxSubSteps, btScalar fixedTimeStep )
{
	...
	// initialize them so don't get key error later
	{
		m_timeStepInfo["ProblemSize"] = json::Number( 0 );
		m_timeStepInfo["FrictionDirs"] = json::Number( m_frictionDirs );
		m_timeStepInfo["NumContacts"] = json::Number( 0 );
		m_timeStepInfo["NumConstraints"] = json::Number( 0 );
		m_timeStepInfo["NumBodies"] = json::Number( 0 );
	}
	...
	if ( numSimulationSubSteps )
	{
		// Profile
		{
			...
			m_LCPsolutionTime = 0; // in microsec
		}
		...
		{
			m_LCPsolutionTime = (long int)(m_clock.getTimeMicroseconds() / clampedSimulationSteps);
			m_timeStepInfo["SolutionTime"] = json::Number((m_clock.getTimeMicroseconds() / clampedSimulationSteps));

			//json::Number jnum = m_timeStepInfo["SolutionTime"];
			//std::cout <<  getJsonInfo() << std::endl;
			//

			// @@@ maybe not here
			std::stringstream ss;
			json::Writer::Write(m_timeStepInfo,ss);
...
/*
  **********************************************************************
   Additional stuffs for ST time-stepper
  ***********************************************************************/
void btSTWorld::generateLocalFrictions()
{
...
void btSTWorld::internalSingleStepSimulation( btScalar timeStep )
{
...
void btSTWorld::setGravity( const btVector3 & gravity )
{
	m_gravity = gravity;
...
btVector3 btSTWorld::getGravity() const
{
	return m_gravity;
} // getGravity

void btSTWorld::addCollisionObject( btCollisionObject* collisionObject,short int collisionFilterGroup,short int collisionFilterMask )
{
	btCollisionWorld::addCollisionObject( collisionObject,collisionFilterGroup,collisionFilterMask );
} // addCollisionObject

void btSTWorld::removeCollisionObject( btCollisionObject* collisionObject )
{
...
void btSTWorld::removeRigidBody( btRigidBody* body )
{
	m_nonStaticRigidBodies.remove( body );
	btCollisionWorld::removeCollisionObject( body );
} // removeRigidBody

void btSTWorld::addRigidBody( btRigidBody* body )
{
...
void btSTWorld::addRigidBody( btRigidBody* body, short group, short mask )
{
	if ( !body->isStaticOrKinematicObject() && !( body->getFlags() & BT_DISABLE_WORLD_GRAVITY ) )
		body->setGravity( m_gravity );
...
void btSTWorld::updateActions( btScalar timeStep )
{
...
void btSTWorld::updateActivationState( btScalar timeStep )
{
...
void btSTWorld::addConstraint( btTypedConstraint* constraint,bool disableCollisionsBetweenLinkedBodies )
{
...
void btSTWorld::removeConstraint( btTypedConstraint* constraint )
{
...
void btSTWorld::addAction( btActionInterface* action )
{
	m_actions.push_back( action );
} // addAction

void btSTWorld::removeAction( btActionInterface* action )
{
	m_actions.remove( action );
} // removeAction

void btSTWorld::addVehicle( btActionInterface* vehicle )
{
	addAction( vehicle );
} // addVehicle

void btSTWorld::removeVehicle( btActionInterface* vehicle )
{
	removeAction( vehicle );
} // removeVehicle

void btSTWorld::addCharacter( btActionInterface* character )
{
	addAction( character );
} // addCharacter

void btSTWorld::removeCharacter( btActionInterface* character )
{
...
void btSTWorld::solveConstraints( btContactSolverInfo & solverInfo )
{
		...
		: public btSimulationIslandManager::IslandCallback
	{
		...
		InplaceSolverIslandCallback( btContactSolverInfo & solverInfo,btConstraintSolver* solver,btTypedConstraint* * sortedConstraints,int numConstraints,btIDebugDraw* debugDrawer,btStackAlloc* stackAlloc,btDispatcher* dispatcher )
...
void btSTWorld::calculateSimulationIslands()
{
	...
	: public btCollisionWorld::ClosestConvexResultCallback
{
	...
	btDispatcher          * m_dispatcher;
	public:	btClosestNotMeConvexResultCallback( btCollisionObject* me,const btVector3 & fromA,const btVector3 & toA,btOverlappingPairCache* pairCache,btDispatcher* dispatcher )
...
int gBTNumClampedCcdMotions = 0;

#define USE_TIMESTEP_LIST 1
// #include "stdio.h"
void btSTWorld::integrateTransforms( btScalar timeStep )
{
	BT_PROFILE( "integrateTransforms" );
	btTransform predictedTrans;

#if USE_TIMESTEP_LIST
	for ( int i = 0; i < m_nonStaticRigidBodies.size(); i++ )
	{
		btRigidBody* body = m_nonStaticRigidBodies[i];
...
#endif //USE_TIMESTEP_LIST
} // integrateTransforms

void btSTWorld::predictUnconstraintMotion( btScalar timeStep )
{
...
void btSTWorld::startProfiling( btScalar timeStep )
{
	(void)timeStep;
...
void btSTWorld::debugDrawConstraint( btTypedConstraint* constraint )
{
...
void btSTWorld::setConstraintSolver( btConstraintSolver* solver )
{
	if ( m_ownsConstraintSolver )
		btAlignedFree( m_constraintSolver );
...
btConstraintSolver * btSTWorld::getConstraintSolver()
{
	return m_constraintSolver;
} // getConstraintSolver

int btSTWorld::getNumConstraints() const
{
	return int( m_constraints.size() );
} // getNumConstraints

btTypedConstraint * btSTWorld::getConstraint( int index )
{
	return m_constraints[index];
} // getConstraint

const btTypedConstraint * btSTWorld::getConstraint( int index ) const
{
	return m_constraints[index];
} // getConstraint

void btSTWorld::serializeRigidBodies( btSerializer* serializer )
{
	int i;
...
void btSTWorld::serialize( btSerializer* serializer )
{
